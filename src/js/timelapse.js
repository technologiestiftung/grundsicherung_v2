...
export function timelapse() {
  ...
  d3.json("config.json")
    .then(function(_config) {
      ...
      d3.json("data/xii.geojson").then(function(data) {
        projection_timelapse = d3.geoMercator().fitSize([width, height], data);
        path_timelapse = d3.geoPath().projection(projection_timelapse);
      });

      timelapse_map = d3
        .select("#timelapse-map")
        .append("svg")
        .attr("width", width)
        .attr("height", height);

      dataLayer_tl = timelapse_map
        .append("g")
      ...
      function update(year) {
        d3.csv("data/timelapse.csv")
          .then(function(data) {
            d3.json("data/lor_planungsraeume.geojson").then(function(json) {
              ...
              dataLayer_tl
                ...
                .attr("fill", function(d) {
                  return isNaN(d.properties.value)
                    ? "#c2c2c2"
