...
export function map() {
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const padding_total =
      parseFloat(style["paddingLeft"].replace("px", "")) * 2;
    ...
    const updateNav = (contentState, mapState) => {
      ...
      navContent.classed("active", (d, i) =>
        i === state.content ? true : false
      );
        ...
        .then(data => {
          ...
          const scale = d3
            ...
            .domain(
              "extent" in mapConfig
                ? mapConfig.extent
                : [0, maxScale / 2, maxScale]
            ) //d3.extent(data.features, (d) => d.properties[mapConfig.attribute]
            .range("colors" in config ? config.colors : ["#2e91d2", "#F4F5F9"]);

          // tooltips
          function mouseover() {
            tooltip_map.style("display", "block").style("position", "absolute");
          }
          function mousemove() {
          ...
          dataLayer
            ...
            .attr("fill", d => {
              return isNaN(d.properties[mapConfig.attribute])
                ? "#c2c2c2"
